...
const savePlayListAPI = async (playlist: Playlist) => await externalAPI.post("/playlists", playlist)
const updatePlayListAPI = async ({ id, playlist }: { id: number; playlist: Playlist }) =>
    await externalAPI.patch(`/playlists/${id}`, playlist)

const getAllPlayListsFx = createEffect<any, AxiosResponse<TResponse<Playlist>>, Error>(
    getAllPlayListsAPI
)
