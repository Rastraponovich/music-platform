...
guard({
  clock: $loop,
  source: [$audio, $loop],
  filter: (params): params is [HTMLAudioElement, boolean] => params[0] instanceof HTMLAudioElement,
...
const $playerState = createStore<PlayerState>(PlayerState.STOPED)
  .on(setPlaying, () => PlayerState.PLAYED)
  .on(setPause, () => PlayerState.PAUSED)
  .on(onStopButtonClicked, () => PlayerState.STOPED)
  .on(destroyPlayer, () => PlayerState.DESTROYED);

const switchTimeMode = createEvent();
const $timeMode = createStore<TimeMode>(TimeMode.ELAPSED).on(switchTimeMode, (state, _) =>
  state === TimeMode.ELAPSED ? TimeMode.REMAINING : TimeMode.ELAPSED,
);
