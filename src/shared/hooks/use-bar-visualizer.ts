 ...
 */
function preRenderBar(height: number, colors: string[], renderHeight: number): HTMLCanvasElement {
  const barCanvas = document.createElement("canvas"); // Off-screen canvas for pre-rendering a single bar gradient
...
export function usePaintBar({ renderHeight, height }: { renderHeight: number; height: number }) {
  ...
  return useCallback(
    (ctx: CanvasRenderingContext2D, x: number, barHeight: number, peakHeight: number) => {
