...
import { duration, progress, winamp, winampControls } from "@/src/widgets/winamp/model";
...
const TrackListItemSmall = ({ track, isCurrentTrack }: TrackListItemProps) => {
  ...
  const currentTrackDuration = useUnit(duration.$currentTrackDuration);
  const currentTrackTime = useUnit(progress.$currentTime);
